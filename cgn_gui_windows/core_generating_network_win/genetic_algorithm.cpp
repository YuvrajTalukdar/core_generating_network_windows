...
void genetic_algorithm::calc_fitness_threaded(void (*print_message)(string),int no_of_threads,vector<chromosome>& population)
{
    if(no_of_threads==1)
    {
        for(int a=0;a<population.size();a++)
    ...
    {   
        ...
        for(int a=0;a<population.size();a+=no_of_threads)
        {
            ...
            for(int b=0;b<no_of_threads;b++)
            {
                ...
                {   break;}
                segment_class segment1(0,a+b,"default_segment",print_message);
                segment1.set_ds(ds);
                segment1.add_f_data(*f_data_vector);
...
chromosome genetic_algorithm::start_genetic_algorithm(void (*print_message)(string),int no_of_threads)
{
    ...
    {   no_of_genes_to_mutate=3;}
    print_message("\nthreads= "+to_string(no_of_threads));
    generate_initial_population();
    //print_population(population);
    //fitness calculation
    print_message("\n\nCalculation initial population fitness...");
    calc_fitness_threaded(print_message,no_of_threads,population);
    int total_value=0;
    for(int a=0;a<ga_iterations;a++)
    {
        ...
        mutation(new_gen);
        //fitness calculation
        calc_fitness_threaded(print_message,no_of_threads,new_gen);
        ...
        print_message("\nIteration: "+to_string(a)+", total_population_fitness: "+to_string(total_value)+", max_fitness: "+to_string(population[0].fitness));
    }
    return population[0];
}
