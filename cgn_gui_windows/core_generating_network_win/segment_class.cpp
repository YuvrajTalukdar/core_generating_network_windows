...
int segment_class::make_prediction_on_user_entered_data(vector<float> data)
{
    //clrscr();
    ...
    fired_neuron_index=propagate(data);
    return ds.elements[fired_neuron_index];
}

int segment_class::get_data_length()
{
    return ds.no_of_elements_in_each_record;
}
...
segment_class::segment_class(int segment_aim1,int segment_no1,string segment_name1,void (*print_message1)(string))
{
    print_message=print_message1;
    if(id_lock==false)
    {
